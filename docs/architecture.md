...
キャプション翻訳 (Helsinki-NLP/opus-mt-en-jap): Florence-2 が生成した英語キャプションを日本語に翻訳します。これにより、日本語に最適化された Sparse Encoder に適切な入力を与えることができます。
...
graph TD
    ...
    F -->|英語キャプション| H1[キャプション翻訳<br/>opus-mt-en-jap]
    H1 -->|日本語キャプション| H2[画像の説明文<br/>「〜の画像」]
...
* **画像キャプション生成 (`microsoft/Florence-2-base-ft`)**
* **役割:** テキストを含まない風景写真などに対し、「〜の画像」といった詳細な自然言語の説明文を生成します。
* **特徴:** これにより、文字情報を持たない画像に対しても、後続の処理で有効なSparse Vectorを生成できるようになります。

* **キャプション翻訳 (`Helsinki-NLP/opus-mt-en-jap`)**
* **役割:** Florence-2 が生成した英語キャプションを日本語に翻訳します。
* **特徴:** 日本語に最適化された Sparse Encoder に適切な入力を与えることで、日本語キーワードによる検索精度を向上させます。
